...
<h1 class="h3 mb-3"><strong>Pi Tool - Categories</strong></h1>
...
<div class="row">
    <div class="col-12 col-lg-12 col-xxl-12 d-flex">
        <div class="card flex-fill">            
            <div class="card-header">
                <h5 class="card-title mb-0">Products List RC</h5>
                <div class="form-check float-end">
                    <input id="linkerFindCategory" class="form-check-input" type="checkbox">
                    <label class="form-check-label">
                        Linker Find Category
                    </label>
                </div>
                <div id="alerts-rc" style="width:100%;"></div>                
            </div>
            <div class="card-body">
                ...
                <table id="productsrc" class="table table-hover my-0">
                    <!-- sku,
					if(title is null or title = \'\',concat(`INV-PRICE-DESC1-1-31`,`INV-PRICE-DESC2`),title) as title,
					m.manufacturer,
					c.category,
    ...
    $(document).ready(function() {
        ...
        tablerc = $('#productsrc').DataTable({
            ...
            "columnDefs": [
                    ...
                    },
                    {
                        ...
                        'render': function (data,type,full,meta){
                            return '<a onclick="rc(\'category\','+full.id+',\''+full.ingram_id+'\')"><span class="badge bg-'+((full.name!='')?'success':'danger')+' c-'+full.ingram_id+' c-label">'+((full.name!='')?full.name:'Linker Category')+'</span></a><div class="ccontainer" id="ccontainer-'+full.ingram_id+'"></div>';
                        }
                    },
                    {
                        targets: [5],
            ...
            "drawCallback": function ( settings ) {
                ...
                api.column(groupColumnRC, {page:'current'} ).data().each( function ( group, i ) {
                    if ( last !== group ) {
                        $(rows).eq( i ).before(
                            '<tr class="group badge bg-success"><td width="100%" colspan="3">'+group+'</td></tr>'
                        );    
