...
<h1 class="h3 mb-3"><strong>Segmentation</strong></h1>
<div class="row">
    <div class="col-12 col-lg-12 col-xxl-12 d-flex">
        <div class="card flex-fill">
                <div class="card-header">
                    <h5 class="card-title mb-0">Segmentation Products <button id="segmentationSave" class="btn btn-success btn-sm float-end btnAdd" data-bs-toggle="modal" data-bs-target="#formSaveModal">Save</button></h5>
        ...
        <div class="card flex-fill">            
            <div class="card-header">
                ...
                <!-- <div class="filter" width="100%"> --> 
                    <select width="50%" id="manufacturer-filter" class="form-control filter-product">            
                        <?php foreach ($Manufacturers as $Manufacturer) : ?>
                            <option value="<?=$Manufacturer["id"];?>"><?=$Manufacturer["manufacturer"];?></option>
                        <?php endforeach; ?>
                    </select>
                
                    <select width="50%" id="categories-filter" class="form-control filter-product">            
                        <?php foreach ($Categories as $Category) : ?>
                            <optgroup label="<?=$Category['parent']["nombre"];?>">
                                <option value="<?=$Category['parent']["id"];?>"><?=$Category['parent']["nombre"]." (All ".((count($Category['child']))?'+ '.count($Category['child']):'').")";?></option>
                                <?php foreach ($Category['child'] as $Child) : ?>
...
<script>
    ...
    $(document).ready(function() {
        ...
        $('#segmentationSave').on('click', function(e) {
            $.post("segmentation/segmentationManufacturerCategories", {
